...
def get_model():
    global model
    if model is None:
        # Build model path relative to the project directory
        base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        model_path = os.path.join(base_dir, 'yolo11n.onnx')
        if not os.path.exists(model_path):
            # Fallback to best.pt if available
            model_path = os.path.join(base_dir, 'best.pt')
        model = YOLO("../backend/yolo11n.onnx")
    return model
            ...
            if hasattr(result.boxes, "conf") and len(result.boxes.conf) > 0:
                ...
                for i, conf in enumerate(confs):
                    if conf > max_confidence:
                        max_confidence = conf
                        detection = {
                            "class": names[i] if i < len(names) else None,
                            "confidence": conf,
